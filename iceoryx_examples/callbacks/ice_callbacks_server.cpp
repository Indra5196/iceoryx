...
constexpr char APP_NAME[] = "iox-cpp-callbacks-server";
...
// void RunRequestHandler(iox::popo::Server<uint32_t, uint32_t>* server){
...
//         else {
//             server->loanResponse(sizeof(uint32_t)).and_then([&](auto& sample) {
//                 *sample = retval;
...
int main()
{
    ...
    iox::popo::Server<uint32_t, uint32_t> myServerLeft({"Radar", "FrontLeft"});
    std::cout << "Server Created\n";
    // myServerLeft.registerRequestHandler(AdjustMethod);

    listener.attachEvent(myServerLeft, iox::popo::ServerEvent::DATA_RECEIVED, iox::popo::createNotificationCallback(myServerLeft.RequestHandler)).or_else([](auto) {
