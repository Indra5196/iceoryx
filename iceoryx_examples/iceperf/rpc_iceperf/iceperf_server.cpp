...
constexpr const char APP_NAME[]{"iceperf-bench-server"};
constexpr const char SERVER[]{"Server"};
//! [use constants instead of magic values]

//! [do the measurement for a single technology]
void IcePerfServer::doMeasurement(IcePerfRPCBase& ipcTechnology) noexcept
{
    ipcTechnology.init();

    ipcTechnology.latencyPerfTestServer();
...
// PerfSettings IcePerfServer::getSettings(iox::popo::Subscriber<PerfSettings>& subscriber) noexcept
// {
...
int IcePerfServer::run() noexcept
{
