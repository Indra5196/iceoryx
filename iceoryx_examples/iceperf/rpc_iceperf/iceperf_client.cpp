...
constexpr const char APP_NAME[]{"iceperf-bench-client"};
constexpr const char CLIENT[]{"Client"};
// constexpr const char SUBSCRIBER[]{"Follower"};
//! [use constants instead of magic values]

IcePerfClient::IcePerfClient(const PerfSettings settings) noexcept
    : m_settings(settings)
{
...
void IcePerfClient::doMeasurement(IcePerfRPCBase& ipcTechnology) noexcept
{
    ipcTechnology.init();
    ...
    for (const auto payloadSizeInKB : payloadSizesInKB)
    {
        ...
        separator = ", ";
        auto payloadSizeInBytes = payloadSizeInKB * IcePerfRPCBase::ONE_KILOBYTE;

        ipcTechnology.preLatencyPerfTestClient(payloadSizeInBytes);

        auto latency = ipcTechnology.latencyPerfTestClient(m_settings.numberOfSamples);

        latencyMeasurements.push_back(std::make_tuple(payloadSizeInKB, latency));

        ipcTechnology.postLatencyPerfTestClient();
    }
    std::cout << std::endl;

    ipcTechnology.releaseServer();
...
int IcePerfClient::run() noexcept
{
