...
PoshRuntime::getMiddlewareClient(const capro::ServiceDescription& service,
                                     const popo::ClientOptions& clientOptions,
                                     const PortConfigInfo& portConfigInfo) noexcept
{
    constexpr uint64_t MAX_QUEUE_CAPACITY = iox::popo::ClientChunkReceiverData_t::ChunkQueueData_t::MAX_CAPACITY;
    ...
    if (options.queueCapacity > MAX_QUEUE_CAPACITY)
    {
        LogWarn() << "Requested queue capacity " << options.queueCapacity
                  ...
                  << ", limiting from " << clientOptions.queueCapacity << " to " << MAX_QUEUE_CAPACITY;
        options.queueCapacity = MAX_QUEUE_CAPACITY;
    }
    else if (0U == options.queueCapacity)
    {
    ...
    constexpr uint64_t MAX_HISTORY_CAPACITY =
        iox::popo::ClientChunkSenderData_t::ChunkDistributorDataProperties_t::MAX_HISTORY_CAPACITY;
    
    if (options.historyCapacity > MAX_HISTORY_CAPACITY)
    {
        LogWarn() << "Requested history capacity " << options.historyCapacity
                  << " exceeds the maximum possible one for this client"
                  << ", limiting from " << clientOptions.historyCapacity << " to " << MAX_HISTORY_CAPACITY;
        options.historyCapacity = MAX_HISTORY_CAPACITY;
    }
    ...
    sendBuffer << IpcMessageTypeToString(IpcMessageType::CREATE_CLIENT) << m_appName
               ...
               << std::to_string(options.queueCapacity) << std::to_string(options.historyCapacity)
               << options.nodeName << std::to_string(options.connectOnCreate)
               << std::to_string(options.fireAndForget)
               << std::to_string(static_cast<uint8_t>(options.queueFullPolicy))
               << std::to_string(static_cast<uint8_t>(options.serverTooSlowPolicy))
               << static_cast<cxx::Serialization>(portConfigInfo).toString();
    ...
    if (maybeClient.has_error())
    {
        ...
        case IpcMessageErrorType::CLIENT_LIST_FULL:
            LogWarn() << "Service '" << service.operator cxx::Serialization().toString()
                      << "' could not be created since we are out of memory for clients.";
            errorHandler(Error::kPOSH__RUNTIME_ROUDI_CLIENT_LIST_FULL, nullptr, iox::ErrorLevel::SEVERE);
            break;
        case IpcMessageErrorType::REQUEST_CLIENT_WRONG_IPC_MESSAGE_RESPONSE:
            LogWarn() << "Service '" << service.operator cxx::Serialization().toString()
                      << "' could not be created. Request clients got wrong IPC channel response.";
        ...
        default:
            LogWarn() << "Undefined behavior occurred while creating service '"
                      << service.operator cxx::Serialization().toString() << "'.";
            errorHandler(
                Error::kPOSH__RUNTIME_CLIENT_PORT_CREATION_UNDEFINED_BEHAVIOR, nullptr, iox::ErrorLevel::SEVERE);
            break;
        }
        return nullptr;
    }
...
PoshRuntime::getMiddlewareServer(const capro::ServiceDescription& service,
                                     const popo::ServerOptions& serverOptions,
                                     const PortConfigInfo& portConfigInfo) noexcept
{
    constexpr uint64_t MAX_QUEUE_CAPACITY = iox::popo::ServerChunkReceiverData_t::ChunkQueueData_t::MAX_CAPACITY;
    ...
    if (options.queueCapacity > MAX_QUEUE_CAPACITY)
    {
        LogWarn() << "Requested queue capacity " << options.queueCapacity
                  ...
                  << ", limiting from " << serverOptions.queueCapacity << " to " << MAX_QUEUE_CAPACITY;
        options.queueCapacity = MAX_QUEUE_CAPACITY;
    }
    else if (0U == options.queueCapacity)
    {
    ...
    constexpr uint64_t MAX_HISTORY_CAPACITY =
        iox::popo::ServerChunkSenderData_t::ChunkDistributorDataProperties_t::MAX_HISTORY_CAPACITY;
    
    if (options.historyCapacity > MAX_HISTORY_CAPACITY)
    {
        LogWarn() << "Requested history capacity " << options.historyCapacity
                  << " exceeds the maximum possible one for this Server"
                  << ", limiting from " << serverOptions.historyCapacity << " to " << MAX_HISTORY_CAPACITY;
        options.historyCapacity = MAX_HISTORY_CAPACITY;
    }
    ...
    sendBuffer << IpcMessageTypeToString(IpcMessageType::CREATE_SERVER) << m_appName
               ...
               << std::to_string(static_cast<uint8_t>(options.queueFullPolicy))
               << std::to_string(static_cast<uint8_t>(options.clientTooSlowPolicy))
               << static_cast<cxx::Serialization>(portConfigInfo).toString();
    ...
    if (maybeServer.has_error())
    {
        ...
        case IpcMessageErrorType::SERVER_LIST_FULL:
            LogWarn() << "Service '" << service.operator cxx::Serialization().toString()
                      << "' could not be created since we are out of memory for servers.";
            errorHandler(Error::kPOSH__RUNTIME_ROUDI_SERVER_LIST_FULL, nullptr, iox::ErrorLevel::SEVERE);
            break;
        case IpcMessageErrorType::REQUEST_SERVER_WRONG_IPC_MESSAGE_RESPONSE:
            LogWarn() << "Service '" << service.operator cxx::Serialization().toString()
                      << "' could not be created. Request servers got wrong IPC channel response.";
        ...
        default:
            LogWarn() << "Undefined behavior occurred while creating service '"
                      << service.operator cxx::Serialization().toString() << "'.";
            errorHandler(
                Error::kPOSH__RUNTIME_SERVER_PORT_CREATION_UNDEFINED_BEHAVIOR, nullptr, iox::ErrorLevel::SEVERE);
            break;
        }
        return nullptr;
    }
...
PoshRuntime::requestClientFromRoudi(const IpcMessage& sendBuffer) noexcept
{
    ...
    if (sendRequestToRouDi(sendBuffer, receiveBuffer) && (3U == receiveBuffer.getNumberOfElements()))
    {
        ...
        if (stringToIpcMessageType(IpcMessage.c_str()) == IpcMessageType::CREATE_CLIENT_ACK)
        {
            ...
            cxx::convert::fromString(receiveBuffer.getElementAtIndex(1U).c_str(), offset);
            auto ptr = rp::BaseRelativePointer::getPtr(segmentId, offset);
            return cxx::success<ClientPortUserType::MemberType_t*>(
                reinterpret_cast<ClientPortUserType::MemberType_t*>(ptr));
        }
    }
    else
    {
...
PoshRuntime::requestServerFromRoudi(const IpcMessage& sendBuffer) noexcept
{
    ...
    if (sendRequestToRouDi(sendBuffer, receiveBuffer) && (3U == receiveBuffer.getNumberOfElements()))
    {
        ...
        if (stringToIpcMessageType(IpcMessage.c_str()) == IpcMessageType::CREATE_SERVER_ACK)
        {
            ...
            cxx::convert::fromString(receiveBuffer.getElementAtIndex(1U).c_str(), offset);
            auto ptr = rp::BaseRelativePointer::getPtr(segmentId, offset);
            return cxx::success<ServerPortUserType::MemberType_t*>(
                reinterpret_cast<ServerPortUserType::MemberType_t*>(ptr));
        }
    }
    else
    {
